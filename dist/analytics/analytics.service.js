...
let AnalyticsService = class AnalyticsService {
    ...
    async getOverview() {
        ...
        const [totalStudents, totalGroups, totalTeachers, newStudentsThisWeek, currentMonthPayments, lastMonthPayments, debtorsData, allActiveStudents, thisMonthAttendance,] = await Promise.all([
    ...
    async getRevenue(query) {
        ...
        const result = MONTH_NAMES.map((month, i) => {
            const monthPayments = payments.filter((p) => p.confirmedAt && new Date(p.confirmedAt).getMonth() === i);
    ...
    async getAttendanceCenter(query) {
        const where = {};
        if (query.groupId)
        ...
        for (const r of records) {
            ...
            if (!monthMap.has(key))
                monthMap.set(key, { p: 0, total: 0 });
            const entry = monthMap.get(key);
            entry.total++;
            if (r.status !== client_1.AttendanceStatus.ABSENT)
                entry.p++;
        }
    ...
    async getDebtors() {
        ...
        return allActiveStudents
            ...
            .sort((a, b) => {
            if (a.daysSinceLastPayment === null)
                return 1;
            if (b.daysSinceLastPayment === null)
                return -1;
        ...
        return teachers
            ...
            for (const g of t.groups) {
                for (const a of g.attendances) {
                    total++;
                    if (a.status !== client_1.AttendanceStatus.ABSENT)
                        present++;
                }
            }
