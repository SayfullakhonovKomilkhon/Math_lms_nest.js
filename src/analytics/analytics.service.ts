...
export class AnalyticsService {
  ...
  async getAttendanceCenter(query: DateRangeQueryDto) {
    ...
    const overallPercent = total > 0 ? Math.round(((present + late) / total) * 100) : 0;

    // By group
    const groupMap = new Map<string, { groupName: string; teacherName: string; p: number; a: number; l: number }>();
    for (const r of records) {
      const key = r.groupId;
      if (!groupMap.has(key)) {
        groupMap.set(key, {
  ...
  async getGradesCenter(query: DateRangeQueryDto) {
    ...
    const toPercent = (score: number, max: number) => max > 0 ? (score / max) * 100 : 0;

    // Center average
    const centerAverage =
      grades.reduce((sum, g) => sum + toPercent(Number(g.score), Number(g.maxScore)), 0) /
      grades.length;

    // By group
    const groupMap = new Map<string, { groupName: string; teacherName: string; scores: number[]; studentIds: Set<string> }>();
    ...
    // By teacher
    const teacherMap = new Map<string, { teacherName: string; groupIds: Set<string>; studentIds: Set<string>; scores: number[] }>();
    for (const g of grades) {
    ...
    // Top students
    const studentMap = new Map<string, { fullName: string; groupName: string; scores: number[] }>();
    for (const g of grades) {
  ...
  async getTeachersLoad() {
    ...
    return teachers
      .map((t) => {
        const studentsCount = t.groups.reduce((sum, g) => sum + g._count.students, 0);
